...
class PoiCont {
    ...
    setPoi(flist, actonly) {
        ...
        const checkMarker = (poi) => {
            ...
            const tg1 = actlists.length > 0 ? [Conf.google.targetName, ...poi.targets] : poi.targets;
            const tg2 = tg1.filter(t => poiCont.getTargets().includes(t));
            const inZoom = tg2.some(t => zoomlv >= Conf.poiView.poiZoom[t] || zoomlv >= Conf.poiView.editZoom[t]);
            if (!viewflag || !inZoom) return null;
            return { poi, actlists };
        };
        ...
        const saveMarker = function (params) {              // params内のgeojsonをgeojson変数に保存
            ...
            let poiGeojson = structuredClone(params.poi.geojson);
            if (poiGeojson.geometry.type !== "Point") {
                poiGeojson.geometry.type = "Point";
                poiGeojson.geometry.coordinates = params.poi.lnglat;
            }
            const osmid = tags.id ?? params.poi.geojson.id;                 // OSMID を確実にセット（promoteId 用）
            poiGeojson.properties.id = poiGeojson.properties.id ?? osmid;
            poiGeojson.id = poiGeojson.id ?? osmid;                         // feature.id も揃える
            geojson[css_name].features.push(poiGeojson);
            if (tags.level !== undefined) {   //2F以上の影つけ
                let level = Number(tags.level)
                if (level > 0) geojson["shadow"].features.push(poiGeojson);
            }
        }
        ...
        const selectMarker = function (e) {
            ...
            let poi = poiCont.get_osmid(props.id)
            let geojson = structuredClone(poi.geojson)  // Point以外の場合はlnglatを利用する
            if (geojson.geometry.type !== "Point") {
                geojson.geometry.type = "Point";
                geojson.geometry.coordinates = poi.lnglat;
            }
        ...
        if (flist !== undefined) {	// set flist
            ...
            flist.forEach(list => {
                ...
                if (osmidx.indexOf(osmid) > -1) {
                    ...
                    osmidx.push(osmid);
                    const params = checkMarker(poi);
                    if (params) saveMarker(params);
                } else {
            ...
            if (allPois.length > 0) {				// acts表示
                allPois.forEach((act) => {
                    ...
                    if (!osm) return;
                    if (!(act.category == org_target || org_target == "activity")) return;
                    const poi = { geojson: osm.geojson, targets: osm.targets, lnglat: osm.lnglat };
                    const params = checkMarker(poi);
                    if (params && params.actlists.length > 0) {
                        poisOrder.unshift(params); // activity を先頭に
                    }
                });
            };
        ...
        markers.forEach(marker => {
            ...
            if (source !== undefined) {
                ...
                mapLibre.map.addSource('marker-' + marker, { type: 'geojson', data: geojson[marker], promoteId: 'id' });
            }

            // 背景アイコンの新規作成（circleへ変更するので削除予定）
            if (!mapLibre.map.getLayer('marker-bg-' + marker)) {
                mapLibre.map.addLayer({
                    id: 'marker-bg-' + marker, type: 'circle', source: 'marker-' + marker,
                    "filter": ["==", ["geometry-type"], "Point"],
                    "paint": {
                        "circle-radius":
                            [
                                'step', ['zoom'], Conf.icon[marker] * 0.5 * 48,
                                16, Conf.icon[marker] * 0.6 * 48,    // ズーム15以上
                                17, Conf.icon[marker] * 0.7 * 48     // ズーム16以上
                            ],
                        "circle-color": mcolors[marker].fill,          // 塗り色（これが fill 相当）
                        "circle-opacity": 1,
                        "circle-stroke-color": mcolors[marker].stroke,   // 枠線色
            ...
            if (!mapLibre.map.getLayer('marker-fg-' + marker)) {
                mapLibre.map.addLayer({
                    ...
                    layout: {
                        'symbol-placement': 'point', 'icon-allow-overlap': true, 'icon-ignore-placement': true,
