...
class Basic {
    ...
    autoLink(str) {
        ...
        var regexp_makeLink = function (all, url, h, href) {
            return '<a href="h' + href + '" target="_new">' + url + '</a>';
        }
    ...
    parseOpeningHours(tag) {
        ...
        for (const rule of rules) {
            ...
            const normalizedTime = timePart.trim().toLowerCase();
            const time = ["off", "closed"].includes(normalizedTime)
                ? glot.get("closed") : timePart;
